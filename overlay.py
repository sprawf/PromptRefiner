"""
Floating status pill — appears near the cursor during refinement.
...
from theme import ACCENT, OK, WARN, ERR, FONT_FAMILY, FONT_MONO
...
class OverlayWindow:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self._win      = None
        self._canvas   = None
        self._main_id  = None     # canvas text item id
        self._bar_id   = None     # left accent bar item id
        self._tick     = False
        self._t0       = 0.0
    ...
    def show_loading_model(self) -> None:
        self._close()
        self._build('⏳  Loading model — please wait', _TEXT_CLR, WARN)
        if self._win:
    ...
    def _build(self, text: str, fg: str, bar_color: str) -> None:
        ...
        cy = self.root.winfo_pointery() + 24
    ...
    def _set_text(self, text: str) -> None:
        if self._canvas and self._main_id:
            self._canvas.itemconfig(self._main_id, text=text)
            # Reposition text based on new width
            new_x = _RADIUS // 2 + 4 + 10 + self._fnt.measure(text) // 2
    ...
    def _update(self) -> None:
        ...
        self._win.after(100, self._update)
